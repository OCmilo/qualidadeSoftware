<template>
  <div>
    <h2 id="page-heading" data-cy="CouponHeading">
      <span v-text="$t('appApp.coupon.home.title')" id="coupon-heading">Coupons</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('appApp.coupon.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CouponCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('appApp.coupon.home.createLabel')"> Create a new Coupon </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && coupons && coupons.length === 0">
      <span v-text="$t('appApp.coupon.home.notFound')">No coupons found</span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="appApp.coupon.delete.question" data-cy="couponDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-coupon-heading" v-text="$t('appApp.coupon.delete.question', { id: removeId })">
